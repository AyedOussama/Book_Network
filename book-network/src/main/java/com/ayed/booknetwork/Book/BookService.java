...
import com.ayed.booknetwork.Exception.OperationNotPermittedException;
import com.ayed.booknetwork.File.FileStorageService;
import com.ayed.booknetwork.History.BookTransactionHistory;
import com.ayed.booknetwork.History.HistoryRepository;
...
public class BookService {
    ...
    private final HistoryRepository transactionHistoryRepository;
    ...
    public PageResponse<BookResponse> findAllBooks(int page, int size, Authentication connectedUser) {
        ...
        org.springframework.data.domain.Page<Book> books = bookRepository.findAllDisplayedBooks(pageable, user.getId());
    ...
    public PageResponse<BookResponse> findAllBooksByOwner(int page, int size, Authentication connectedUser) {
        User user = ((User) connectedUser.getPrincipal());
        Pageable pageable = PageRequest.of(page, size, Sort.by("createdDate").descending());
        org.springframework.data.domain.Page<Book> books = bookRepository.findAll(withOwnerId(user.getId()), pageable);
        List<BookResponse> booksResponse = books.stream()
                .map(bookMapper::toBookResponse)
                .toList();
        return new PageResponse<>(
    ...
    public PageResponse<BorrowedBookResponse> findAllBorrowedBooks(int page, int size, Authentication connectedUser) {
        User user = ((User) connectedUser.getPrincipal());
        Pageable pageable = PageRequest.of(page, size, Sort.by("createdDate").descending());
        org.springframework.data.domain.Page<BookTransactionHistory> allBorrowedBooks = (org.springframework.data.domain.Page<BookTransactionHistory>) transactionHistoryRepository.findAllBorrowedBooks(pageable, user.getId());
    ...
    public PageResponse<BorrowedBookResponse> findAllReturnedBooks(int page, int size, Authentication connectedUser) {
        User user = ((User) connectedUser.getPrincipal());
        Pageable pageable = PageRequest.of(page, size, Sort.by("createdDate").descending());
        org.springframework.data.domain.Page<BookTransactionHistory> allBorrowedBooks = (org.springframework.data.domain.Page<BookTransactionHistory>) transactionHistoryRepository.findAllReturnedBooks(pageable, user.getId());
        List<BorrowedBookResponse> booksResponse = allBorrowedBooks.stream()
                .map(bookMapper::toBorrowedBookResponse)
                .toList();
        return new PageResponse<>(
